  ...
  indiegala() {
    ...
    const process = () => {
      ...
      $('.game-key-string').each((i, ele) => {
        ...
        if (d.wished) $ele.addClass('SBSE-item--wished'); // append icon

        $a.after($('<span class="SBSE-icon"></span>').mouseenter(keylolTooltip.show.bind(keylolTooltip)));
        tooltipsData.push(d);
    ...
    $('#library-contain').eq(0).before($container); // support for new password protected gift page
