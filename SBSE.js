                    ...
                    if (Object.keys(APIData).length > 0) {
                        ...
                        const matchGame = (data) => {
                            if (has.call(data, 'steam') && data.steam.id) {
                                ...
                                $gameTitle.find('.game-name').append(
                                    `<span> | </span><a class="SBSE_steamStore" href="https://store.steampowered.com/app/${d.app}/" target="_blank">${i18n.get('steamStore')}</a>`,
                                    $('<span class="SBSE_icon"></span>').mouseenter(steamCNTooltip.show.bind(steamCNTooltip)),
                                );
    ...
    plati() {
        ...
        if (location.pathname.startsWith('/seller/')) {
                    ...
                    $trs
                        ...
                        .each((i, tr) => {
                            ...
                            if (url.length > 0 && id > 0) {
                                ...
                                if (classes.length > 0) {
                                    $tr
                                        .removeClass('SBSE_owned SBSE_wished SBSE_ignored SBSE_notOwned SBSE_notApplicable')
            ...
            const fetchNextPage = async () => {
                ...
                if (params.id_s > 0) {
                    const res = await fetch(`/asp/block_goods_s.asp?${$.param(params)}`);
            ...
            const scrollHandler = () => {
                if ($body.hasClass('enablePlatiFeature') &&
                    ...
                    infiniteScroll.loading === false &&
                    infiniteScroll.reachedLastPage === false) {
            ...
            const initTable = () => {
                ...
                $table
                    .addClass(filters.join(' ')) // apply filters
